...
describe("DownloadDialog.vue", () => {
  ...
  it("Inits ___ success", async() => {
    ...
    expect(EntityService.getPartialEntity).toHaveBeenCalledTimes(1);
    expect(EntityService.getPartialEntity).toHaveBeenCalledWith("http://snomed.info/sct#298382003", [RDFS.LABEL]);
    expect(wrapper.vm.concept).toStrictEqual({"@id":"http://snomed.info/sct#298382003","http://www.w3.org/2000/01/rdf-schema#label":"Scoliosis deformity of spine (disorder)"});
    ...
    expect(wrapper.vm.parents).toStrictEqual([{"name":"Curvature of spine (disorder)","hasChildren":false,"type":[{"name":"Class","@id":"http://www.w3.org/2002/07/owl#Class"}],"@id":"http://snomed.info/sct#64217002"},{"name":"Disorder of musculoskeletal system (disorder)","hasChildren":false,"type":[{"name":"Class","@id":"http://www.w3.org/2002/07/owl#Class"}],"@id":"http://snomed.info/sct#928000"}]);
    ...
    expect(wrapper.vm.members).toStrictEqual({"valueSet":{"name":"Family history","@id":"http://endhealth.info/im#VSET_RecordType_FamilyHistory"},"included":[{"entity":{"name":"Family history with explicit context (situation)","@id":"http://snomed.info/sct#57177007"},"code":"57177007","scheme":{"name":"Snomed-CT code","@id":"http://endhealth.info/im#SnomedCodeScheme"}}],"excluded":[{"entity":{"name":"No family history of clinical finding (situation)","@id":"http://snomed.info/sct#160266009"},"code":"160266009","scheme":{"name":"Snomed-CT code","@id":"http://endhealth.info/im#SnomedCodeScheme"}}],"limited":false});
  ...
  it("Inits ___ fail", async() => {
    ...
    expect(mockToast.add).toHaveBeenNthCalledWith(2, LoggerService.error("Failed to get parents data from server"));
    expect(mockToast.add).toHaveBeenNthCalledWith(3, LoggerService.error("Failed to get children data from server"));
  ...
  it("Can setIncludeBooleans ___ members", async() => {
    ...
    expect(wrapper.vm.includeParents).toBe(true);
    expect(wrapper.vm.includeChildren).toBe(true);
