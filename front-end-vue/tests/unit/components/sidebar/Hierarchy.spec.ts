...
describe("Hierarchy.vue ___ DiscoveryOntology", () => {
    ...
    EntityService.getPartialEntity = jest.fn().mockResolvedValue({
      data: {
        ...
        "http://www.w3.org/2000/01/rdf-schema#comment":
          "A folder of ontologies including information models, value sets, query sets and maps",
      ...
      data: [
        {
          ...
          hasChildren: true,
          type: [
            { name: "Value set", "@id": "http://endhealth.info/im#ValueSet" }
          ],
  ...
  it("calls services, populates conceptAggregate and updatesHistory on mounted", async () => {
    ...
    expect(
      EntityService.getPartialEntity
    ).toHaveBeenCalledWith("http://endhealth.info/im#DiscoveryOntology", [
      "http://www.w3.org/2000/01/rdf-schema#label",
      "http://www.w3.org/2000/01/rdf-schema#comment",
      "http://www.w3.org/1999/02/22-rdf-syntax-ns#type"
    ]);
    expect(EntityService.getEntityChildren).toHaveBeenCalledTimes(1);
    expect(EntityService.getEntityChildren).toHaveBeenCalledWith(
      "http://endhealth.info/im#DiscoveryOntology"
    );
    expect(EntityService.getEntityParents).toHaveBeenCalledTimes(1);
    expect(EntityService.getEntityParents).toHaveBeenCalledWith(
      "http://endhealth.info/im#DiscoveryOntology"
    );
    await flushPromises();
  ...
  it("fires a toast on conceptAggregate fetch error", async () => {
    EntityService.getEntityChildren = jest
      .fn()
      .mockRejectedValue({ code: 400, error: "Test error" });
  ...
  it("handles conceptIri changes", async () => {
    ...
    wrapper.vm.$options.watch.conceptIri.call(
      wrapper.vm,
      "http://endhealth.info/im#DiscoveryOntology"
    );
    await wrapper.vm.$nextTick();
    expect(wrapper.vm.getConceptAggregate).toHaveBeenCalledTimes(1);
    expect(wrapper.vm.getConceptAggregate).toHaveBeenCalledWith(
      "http://endhealth.info/im#DiscoveryOntology"
    );
    await flushPromises();
    await wrapper.vm.$nextTick();
    expect(wrapper.vm.refreshTree).toHaveBeenCalledTimes(2);
    expect(mockStore.commit).toHaveBeenCalledTimes(0);
  });
  ...
  it("can resetConcept", async () => {
    ...
    expect(wrapper.vm.sideNavHierarchyFocus.iri).toEqual(
      "http://endhealth.info/im#DiscoveryOntology"
    );
...
describe("Hierarchy.vue ___ Concept", () => {
            ...
            "http://endhealth.info/im#oneOf": [
              {
                ...
                "http://endhealth.info/im#mapAdvice":
                  "ALWAYS M41.9 | FIFTH CHARACTER POSSIBLE",
              ...
              },
              {
                ...
                "http://endhealth.info/im#mapAdvice":
                  "ALWAYS M41.8 | FIFTH CHARACTER POSSIBLE",
                "http://endhealth.info/im#mapPriority": {
                  "@value": 2,
                  "@type": "http://www.w3.org/2001/XMLSchema#integer"
                },
  ...
  it("calls services, populates conceptAggregate and updatesHistory on mounted", async () => {
    ...
    expect(
      EntityService.getPartialEntity
    ).toHaveBeenCalledWith("http://snomed.info/sct#298382003", [
      "http://www.w3.org/2000/01/rdf-schema#label",
      "http://www.w3.org/2000/01/rdf-schema#comment",
      "http://www.w3.org/1999/02/22-rdf-syntax-ns#type"
    ]);
  ...
  it("handles conceptIri changes ___ not home", async () => {
    ...
    expect(mockStore.commit).toHaveBeenLastCalledWith("updateHistory", {
      conceptName: "Scoliosis deformity of spine (disorder)",
      url: "/concept/http:%2F%2Fsnomed.info%2Fsct%23298382003",
      view: "Concept"
    });
  ...
  it("createsTree ___ no root", () => {
    wrapper.vm.refreshTree = jest.fn();
    wrapper.vm.refreshTree(
      EntityService.getPartialEntity,
      EntityService.getEntityParents,
      EntityService.getEntityChildren
    );
    expect(wrapper.vm.refreshTree).toHaveBeenCalledTimes(1);
  });
  ...
  it("can expand children", async () => {
    ...
    expect(EntityService.getEntityChildren).toHaveBeenCalledWith(
      "http://endhealth.info/im#TestConcept"
    );
  ...
  it("can expand children ___ api fail", async () => {
    ...
    EntityService.getEntityChildren = jest
      .fn()
      .mockRejectedValue({ code: 404, message: "testError" });
    const testNode = {
      data: "http://endhealth.info/im#TestConcept",
      key: "testKey",
      loading: false,
  ...
  it("can check containsChild ___ false", async () => {
    expect(
      wrapper.vm.containsChild([{ data: "TestFail" }], { "@id": "TestId" })
    ).toBeFalsy();
  });

  it("can check containsChild ___ true", async () => {
    expect(
      wrapper.vm.containsChild([{ data: "TestId" }], { "@id": "TestId" })
    ).toBeTruthy();
  });
  ...
  it("can expand parents __ api pass", async () => {
    ...
    expect(EntityService.getEntityParents).toHaveBeenNthCalledWith(
      1,
      "http://snomed.info/sct#298382003"
    );
    expect(EntityService.getEntityParents).toHaveBeenLastCalledWith(
  ...
  it("can expand parents __ api 1 fail", async () => {
    ...
    expect(mockToast.add).toHaveBeenCalledTimes(1);
    expect(mockToast.add).toHaveBeenCalledWith(
      LoggerService.error("Concept parents server request 1 failed")
    );
  });
  ...
  it("can expand parents __ api 2 fail", async () => {
    ...
    expect(mockToast.add).toHaveBeenCalledTimes(1);
    expect(mockToast.add).toHaveBeenCalledWith(
      LoggerService.error("Concept parents server request 2 failed")
    );
  });
