...
describe("Hierarchy.vue ___ DiscoveryOntology", () => {
  ...
  beforeEach(() => {
    jest.resetAllMocks();
    EntityService.getEntity = jest.fn().mockResolvedValue({data:{
  ...
  it("calls services, populates conceptAggregate and updatesHistory on mounted", async() => {
    expect(EntityService.getEntity).toHaveBeenCalledTimes(1);
    expect(EntityService.getEntity).toHaveBeenCalledWith("http://endhealth.info/im#DiscoveryOntology");
...
describe("Hierarchy.vue ___ Concept", () => {
  ...
  it("calls services, populates conceptAggregate and updatesHistory on mounted", async() => {
    expect(EntityService.getEntity).toHaveBeenCalledTimes(1);
    expect(EntityService.getEntity).toHaveBeenCalledWith("http://snomed.info/sct#298382003");
    expect(EntityService.getEntityChildren).toHaveBeenCalledTimes(1);
