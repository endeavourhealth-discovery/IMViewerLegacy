...
describe("login.vue with registeredUser", () => {
  ...
  it("checks for existing avatar and sets user avatar ___ false", async() => {
    const legacyUser = { username: "legacy", avatar: { value: "image.jpg" } };
    ...
    await wrapper.vm.$nextTick();
    await flushPromises();
    expect(mockStore.commit).toHaveBeenNthCalledWith(1,"updateCurrentUser", { username: "legacy", avatar: { value: "colour/001-man.png" } });
  });
  ...
  it("fires swal on auth error ___ 401", async() => {
    ...
    await flushPromises();
    expect(Swal.fire).toBeCalledTimes(1);
    expect(Swal.fire).toBeCalledWith({ icon: "warning", title: "User Unconfirmed", text: "Account has not been confirmed. Please confirm account to continue.", showCloseButton: true, showCancelButton: true, confirmButtonText: "Confirm Account" });
  });
