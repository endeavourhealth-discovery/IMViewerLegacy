<template>
  <div class="parent-item-container" :id="id">
    <div class="label-container">
      <span class="float-text">Parent</span>
      <InputText
        ref="miniSearchInput"
        type="text"
        v-model="searchTerm"
  ...
  methods: {
    ...
    async search(): Promise<void> {
      if (this.searchTerm.length > 0) {
        ...
        searchRequest.typeFilter.push(IM.CONCEPT, IM.CONCEPT_SET);
    ...
    addNextClicked(): void {
      this.$emit("addNextOptionsClicked", {
        previousComponentType: DefinitionType.PARENT,
        previousPosition: this.position,
        parentGroup: DefinitionType.PARENT
      });
    }
  }
});
