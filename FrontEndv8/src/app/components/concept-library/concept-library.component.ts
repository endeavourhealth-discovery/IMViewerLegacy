...
export class ConceptLibraryComponent implements OnInit {
  ...
  ngOnChanges(): void {
    this.perspective = this.perspectiveService.getPerspectiveByConceptType(
      this.concept.conceptType
    );
    /*
    ...
    this.service.isValuesetMember(conceptIri, this.concept.iri).subscribe(
      (result) => {
        if (!result.includedBy) { 
          ...
          alert(
            'Included as descendant of [' +
              result.includedBy.name +
              ']\nbut further excluded as descendant of [' +
              result.excludedBy.name +
              ']'
          );
        }
      },
