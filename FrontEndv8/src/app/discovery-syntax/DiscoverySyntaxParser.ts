...
export class DiscoverySyntaxParser extends Parser {
		...
		try {
			...
			if (_la===DiscoverySyntaxParser.SC) {
				...
				while (_la===DiscoverySyntaxParser.SC) {
					...
					this._errHandler.sync(this);
					_la = this._input.LA(1);
					if (((((_la - 41)) & ~0x1F) === 0 && ((1 << (_la - 41)) & ((1 << (DiscoverySyntaxParser.SUBCLASS - 41)) | (1 << (DiscoverySyntaxParser.EQUIVALENTTO - 41)) | (1 << (DiscoverySyntaxParser.DISJOINT - 41)))) !== 0)) {
						{
						this.state = 134;
						this.classAxiom();
						}
	...
	public iri(): IriContext {
		...
		try {
			...
			this.state = 345;
			_la = this._input.LA(1);
			if ( !(((((_la - 52)) & ~0x1F) === 0 && ((1 << (_la - 52)) & ((1 << (DiscoverySyntaxParser.PREFIXIRI - 52)) | (1 << (DiscoverySyntaxParser.IRIREF - 52)) | (1 << (DiscoverySyntaxParser.QUOTED_STRING - 52)))) !== 0)) ) {
			this._errHandler.recoverInline(this);
			} else {
				if (this._input.LA(1) === Token.EOF) {
					this.matchedEOF = true;
...
export class AnnotationsContext extends ParserRuleContext {
	...
	@Override public get ruleIndex(): number { return DiscoverySyntaxParser.RULE_annotations; }
	@Override
	public enterRule(listener: DiscoverySyntaxListener): void {
...
export class RecordTypeContext extends ParserRuleContext {
	public RECORD(): TerminalNode { return this.getToken(DiscoverySyntaxParser.RECORD, 0); }
	public classAxiom(): ClassAxiomContext[];
	public classAxiom(i: number): ClassAxiomContext;
	public classAxiom(i?: number): ClassAxiomContext | ClassAxiomContext[] {
		if (i === undefined) {
