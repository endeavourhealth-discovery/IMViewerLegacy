...
export class ValueSetTree {
  ...
  static NODE_SELECTED_EVENT: string = "app:valueSetTree-node-selected";
  ...
  private dataChange: BehaviorSubject<ConceptReferenceNode[]> = new BehaviorSubject([]);
  ...
  init(tree: ConceptReferenceNode, selectedIri: string) {
    ...
    let selectedNode = this.getNode(selectedIri);
    if(selectedNode != null) {
      this.selectedIri = selectedIri;
      this.expand(selectedNode);
    }
    else {
      this.log.error(`Unable to find node with iri ${selectedIri} in the tree`);
    }
  }  
  ...
  selectNode(node: ConceptReferenceNode):void {
    ...
    this.eventBus.cast(ValueSetTree.NODE_SELECTED_EVENT, node.iri);
  }
  ...
  private initTree(tree: ConceptReferenceNode): void {
    this.addNode(tree);
    
    // attach parents
    this.initAncestors(tree);

    // attach children
    this.initDecendants(tree);
  }
  ...
  private getNode(iri: string): ConceptReferenceNode {
    return this.nodes.get(iri);
  }

  private addNode(node: ConceptReferenceNode): boolean {
